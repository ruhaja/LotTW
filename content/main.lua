...
local modinNimi = RegisterMod("Lament of the Tormented Witch",1)
local game = Game()
local musicManager = MusicManager()
local SFXManager = SFXManager()

SoundEffect.SOUND_TESTISOUNDI = Isaac.GetSoundIdByName("testiSoundi")
SoundEffect.SOUND_HERP = Isaac.GetSoundIdByName("herp")
Music.MUSIC_TESTIMUSA = Isaac.GetMusicIdByName("testiMusa")
...
local osculumInfame = Isaac.GetItemIdByName("Osculum Infame")
...
local costumeWitch 				= Isaac.GetCostumeIdByPath("gfx/characters/witch.anm2")
local costumeHereticsFork		= Isaac.GetCostumeIdByPath("gfx/characters/hereticsfork.anm2")
--local costumeOsculumInfame		= Isaac.GetCostumeIdByPath("gfx/characters/osculuninfame.anm2")		tälle jotain?
...
function modinNimi:pickupPassiveItem(player, flag)
	...
	--OSCULUM INFAME
	--if player:HasCollectible(osculumInfame) then
...
function modinNimi:onUpdate()
	...
	--osculum infame testiä
	for _, entity in pairs(Isaac.GetRoomEntities()) do
		local data = entity:GetData()
		entity = entity:ToNPC()		-- =nil, if entity isn't an npc
		if entity and
		entity:IsActiveEnemy(true)
	then
		if entity:IsDead() and
			not data.Died
		then
			data.Died = true
			--do the thing
			elseif entiy.ParentNPC then --for Pin, Larry Jr etc
				data.Died = true
			end
		end
	end
	...
	if not isWitch then
		...
		if WitchItemCount >= 1 then
				isWitch = true
				player:AddBlackHearts(1)
				player:AddNullCostume(costumeWitch)
				PlaySoundAtPos(SoundEffect.SOUND_POWERUP_SPEWER, 1.0, player.Position)
				--transform teksti jotenkin näkyviin
		end	
	end
...
function modinNimi:use_book( )
	...
	SFXManager:Play(SoundEffect.SOUND_TESTISOUNDI, 1, 0, false, 1)
...
--function modinNimi:burnEffect(entity, amt, flag, source, countdown)
	--if source.Type == EntityType.ENTITY_TEAR
	--and source Variant == TearVariant.FIERY TODO
	--then
	--	entity::AddBurn(EntityRef(player), 124, 150)
	--end
--end

--function modinNimi:onUpdeit(player)
--	if musicManager:GetCurrentMusicID() ~= Music.MUSIC_TESTIMUSA then
--		musicManager:Play(Music.MUSIC_TESTIMUSA, 0.5)	--doesn't respect music volume from options
--	end
--end
...
--modinNimi:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, Mod.burnEffect)
...
--modinNimi:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, modinNimi.onUpdeit)	--musiikki testi
